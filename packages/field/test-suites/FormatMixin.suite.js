...
export function runFormatMixinSuite(customConfig) {
  ...
  describe(`FormatMixin ${cfg.suffix ? `(${cfg.suffix})` : ''}`, async () => {
    ...
    it('reflects back formatted value to user on leave', async () => {
      ...
      const generatedValue = generateValueBasedOnType();
      mimicUserInput(formatEl, generatedValue);
      expect(formatEl.inputElement.value).to.not.equal(`foo: ${generatedValue}`);
      // user leaves field
      formatEl.inputElement.dispatchEvent(new CustomEvent(formatEl.formatOn, { bubbles: true }));
      await aTimeout();
      expect(formatEl.inputElement.value).to.equal(`foo: ${generatedValue}`);
    });
    ...
    describe('parsers/formatters/serializers', () => {
      ...
      it('will only call the formatter for valid values on `user-input-changed` ', async () => {
        ...
        const generatedValue = generateValueBasedOnType();
        const generatedValueAlt = generateValueBasedOnType({ toggleValue: true });
        ...
        mimicUserInput(el, generatedValueAlt);

        expect(formatterSpy.callCount).to.equal(1);
        expect(el.formattedValue).to.equal(generatedValueAlt);

        /* el.errorState = false;
        mimicUserInput(el, generatedValue);
        expect(formatterSpy.callCount).to.equal(2);

        expect(el.formattedValue).to.equal(`foo: ${generatedValue}`); */
      });
    });
